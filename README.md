...
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/clrty-mis-verify`](https://github.com/clarity-fintech/clrty-mis-verify)
...
**CLRTY MIS Verify** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/clrty-mis-verify
cd clrty-mis-verify
...
| `bin/misc mis/kernel/ClrtyMisVerifyKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/ClrtyMisVerifyKernel.mis --check --compact-letters
...
```
mis/kernel/ClrtyMisVerifyKernel.mis
mis/packs/bytecode_hash.mis
mis/packs/commit_bind.mis
mis/packs/refuse_foreign.mis
mis/packs/static_ml_matrix.mis
mis/packs/vector_attest.mis
...
mis/commands/ClrtyMisVerifyCommandCatalog.mis
mis/commands/ClrtyMisVerifyCommands.mis
mis/commands/ClrtyMisVerifyNetworkBind.mis
...
`mis/kernel/ClrtyMisVerifyKernel.mis` — the kernel declares the invariants the compiler enforces on every check:
...
// Title: CLRTY MIS Verify Kernel

module ClrtyMisVerifyKernel {
  ...
  invariant domain_bound: domain == mis_verify;
  ...
  invariant pack_commit_bind_flag: pack_commit_bind_bound == true;
  invariant pack_bytecode_hash_flag: pack_bytecode_hash_bound == true;
  invariant pack_static_ml_matrix_flag: pack_static_ml_matrix_bound == true;
  invariant pack_vector_attest_flag: pack_vector_attest_bound == true;
  invariant pack_refuse_foreign_flag: pack_refuse_foreign_bound == true;
...
// Creator: Chandler William Ferguson
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: CLRTY MIS Verify · BytecodeHash

module ClrtyMisVerifyBytecodeHash {
  invariant letter_hash_bound: letter_hash_root != @0;
  invariant deep_root_moniversive: deep_root == moniversive;
  invariant settlement_chain: chain_id == 1202;
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_bytecode_hash_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant bytecode_hash_021: bytecode_hash_021_bound == true;
  invariant bytecode_hash_022: bytecode_hash_022_bound == true;
  invariant bytecode_hash_023: bytecode_hash_023_bound == true;
  invariant bytecode_hash_024: bytecode_hash_024_bound == true;
  invariant bytecode_hash_025: bytecode_hash_025_bound == true;
  invariant bytecode_hash_026: bytecode_hash_026_bound == true;
  invariant bytecode_hash_027: bytecode_hash_027_bound == true;
  invariant bytecode_hash_028: bytecode_hash_028_bound == true;
```
...
```
CONNECTED clrty-1/1202 clrty-mis-verify
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/ClrtyMisVerifyCommands.mis
```
...
{
  "ok": true,
  "module": "ClrtyMisVerifyBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1155,
...
*Reproduce:* `make check` → `"ok": true`, module `ClrtyMisVerifyBacklinkIndex`, 9 invariants, 4 outcomes, 1155 typed letters.
...
- [clarity-fintech/clrty-mis-verify](https://github.com/clarity-fintech/clrty-mis-verify) — this repository
